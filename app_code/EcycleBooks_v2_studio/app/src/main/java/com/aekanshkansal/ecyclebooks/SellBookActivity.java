...
public class SellBookActivity extends Activity {
	...
	String name,author,publication,mrp,price,edition;
	EditText bkname,bkauthor,bkpublication,bkmrp,bkprice,bkedition;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_sell_book);
		bkname=(EditText)findViewById(R.id.EditText1);
		bkauthor=(EditText)findViewById(R.id.EditText2);
		bkpublication=(EditText)findViewById(R.id.EditText3);
		...
		sell.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				name=bkname.getText().toString();
				author=bkauthor.getText().toString();
				publication=bkpublication.getText().toString();
				mrp=bkmrp.getText().toString();
				price=bkprice.getText().toString();
				edition=bkedition.getText().toString();
				
				if(name.isEmpty() || author.isEmpty() || mrp.isEmpty() || price.isEmpty() || edition.isEmpty())
				{
				...
				else if(edition.length()>8) {
					MyGlobal.showAlert(SellBookActivity.this, "Notice", "Edition can only be 8 characters");
				}
				else if(name.length()>150 || author.length()>150 || publication.length()>150)
				{
					MyGlobal.showAlert(SellBookActivity.this,"Notice","Length of fields should be in limits");
				}
				else
				{
	...
	class PostAtServer extends AsyncTask<String, Void, JSONObject>	{
		...
		private static final String POST_SELL_URL="http://ecyclebooks.com/sellbook.php";
		@Override
		protected void onPreExecute() {
			super.onPreExecute();
